...
- Gateway（SQLite）自動保存 / 再開
- 簡易入力（心理質問60問）
- 詳細入力（14セクション + 心理質問150問）
- 自由意志の記入内容リセット
- 簡易入力・詳細入力への参考URL追加、元形式の文書・文書ZIP取り込み（時系列情報付き）
...
入力データはagent-home Gatewayのアプリ専用SQLiteリソースへ自動保存します。PCとスマートフォンは同じGatewayの状態を参照します。外部DB、認証、LLM API、OpenAI API等は使用しません。

大政奉還を読み込んだ場合、可能な限り元の `taiseihoukan.zip` 自体をGatewayへ保持します。状態監査履歴は別データとして蓄積し、バックアップ時に両者を束ねます。

「記入内容をリセット」は自由意志側の簡易入力・詳細入力・心理回答・進捗だけを初期化します。読み込み済みの大政奉還データと状態監査履歴は削除しません。

参考URL・取り込んだ文書は自由意志入力の一部としてGatewayへ保存されます。URLはURLのまま、PDF・DOC・DOCXなどの文書は本文を抽出せず元ファイルのまま自由意志ZIPへ添付します。URLや文書内の指示はアプリでは実行せず、外部分析用の情報として扱います。文書はイベント日・記載日が分かる場合はその時系列を優先して解釈します。
...
状態監査履歴はGatewayへ自動保存されます。現在有効な大政奉還と監査履歴だけをまとめたバックアップZIPも用意できます。
...
5. 回答ごとに事後分布を更新
6. **4問目以降、主状態の順位が直前の回答前後で安定した時点で早期終了**
7. 主状態の確率40%以上、2位との差10ポイント以上、次質問後の主状態維持率70%以上を通常判定の条件とする。副状態は確率分布から参考候補として決定し、主状態と同じ順位安定条件は課さない。12問目で必ず終了する
...
状態監査モデルは状態8〜12、質問32〜48を基本とします。実際の監査は固定問数ではなく、現在性の高い質問を優先しながら、主状態が安定した時点で終了します。最低4問・有効回答4件を安全ガードとし、通常は6〜8問程度を目標にします。主状態の通常判定条件は40%以上・2位との差10ポイント以上・次質問後の主状態維持率70%以上です。副状態は確率分布から参考候補として決定します。12問目で必ず最上位候補を判定します。
